            ...
            steps {
                script {
                    ...
                    checkout([$class: 'GitSCM', 
                              ...
                              extensions: [], 
                              submoduleCfg: [], 
                              userRemoteConfigs: [[url: 'https://github.com/cyse7125-fall2023-group2/kafka-consumer-helm', 
                ...
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'GITHUB_CREDENTIALS_ID', usernameVariable: 'githubUsername', passwordVariable: 'githubToken')]) {
                      withEnv(["GH_TOKEN=${githubToken}"]){
                        ...
                        sh "ls"
                        def tagExists = sh(returnStatus: true, script: "gh release view ${newVersion} --repo cyse7125-fall2023-group2/kafka-consumer-helm")
                        if (tagExists != 0) {
